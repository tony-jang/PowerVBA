...
namespace PowerVBA.Core.CodeEdit.CodeCompletion
{
    class VBACompletionEngine
    {
        ...
        public IEnumerable<ICompletionData> GetCompletionData(int offset, bool controlSpace)
        {
            ...
            if (Regex.IsMatch(s, Pattern.pattern3))
            {
                flags = VisibleItem.None;
                CompFlag = CompletionFlag.MethodNaming;
            }
            else if (Regex.IsMatch(s, Pattern.pattern1))
            {
                flags = VisibleItem.Declarer;
                CompFlag = CompletionFlag.OnlyNaming;   
            }
            else if (Regex.IsMatch(s, Pattern.pattern1_1))
            {
                flags = VisibleItem.Declarer;
                CompFlag = CompletionFlag.Naming;
            }
            else if (Regex.IsMatch(s, Pattern.pattern1_2))
            {
                flags = VisibleItem.Declarer;
                CompFlag = CompletionFlag.General;
            }
            else if (Regex.IsMatch(s, Pattern.pattern1_3))
            {
                flags = VisibleItem.Declarer;
                CompFlag = CompletionFlag.As;
            }
            else if (Regex.IsMatch(s, Pattern.pattern2))
            {
                flags = VisibleItem.Declarer;
                CompFlag = CompletionFlag.General;
            }
            else if (Regex.IsMatch(s, Pattern.pattern4))
            {
            ...
            if (flags.HasFlag(VisibleItem.Type))
            {
                ...
                var itms = (new CompletionData[] {
                           ...
                           new CompletionData("Boolean", "Boolean", "참 또는 거짓의 값에 대해서 나타냅니다.",
                               ResourceImage.GetIconImage("DeclaratorIcon"), CompletionFlag.None),
                           new CompletionData("Date", "Date", "날짜 형식의 데이터를 나타냅니다.",
                               ResourceImage.GetIconImage("DeclaratorIcon"), CompletionFlag.None),
                           ...
                           new CompletionData("Long", "Long", "64비트의 부호 있는 숫자를 나타냅니다.",
                               ResourceImage.GetIconImage("DeclaratorIcon"), CompletionFlag.None),
                           new CompletionData("Byte", "Byte", "0에서 255까지의 값을 저장 가능합니다.",
                               ResourceImage.GetIconImage("DeclaratorIcon"), CompletionFlag.None),
                           new CompletionData("Currency", "Currency", "64비트의 부호 있는 숫자를 나타냅니다.",
                               ResourceImage.GetIconImage("DeclaratorIcon"), CompletionFlag.None),
                           new CompletionData("Double", "Double", "64비트의 부호 있는 숫자를 나타냅니다.",
                               ResourceImage.GetIconImage("DeclaratorIcon"), CompletionFlag.None),
                           new CompletionData("LongPtr", "LongPtr", "32비트의 부호 있는 숫자를 나타냅니다.",
                               ResourceImage.GetIconImage("DeclaratorIcon"), CompletionFlag.None),
                           new CompletionData("Single", "Single", "단정 밀도 부동 소수점 형식입니다.",
                               ResourceImage.GetIconImage("DeclaratorIcon"), CompletionFlag.None),

                           new CompletionData("Object", "Object", "어떤 형식이건 저장이 가능합니다.",
                               ResourceImage.GetIconImage("DeclaratorIcon"), CompletionFlag.None)});
            ...
            if (flags.HasFlag(VisibleItem.Declarer))
            {
                ...
                var itms = (new CompletionData[] {
                           ...
                           new CompletionData("Private", "Private", "프로그래밍 요소를 선언한 모듈, 클래스에서만 해당 프로그래밍 요소를 액세스할 수 있도록 지정합니다.",
                               ResourceImage.GetIconImage("DeclaratorIcon"), CompletionFlag.Declarator | CompletionFlag.General),
                           new CompletionData($"Err_Check:{Environment.NewLine}\tIf Err.Number <> 0 Then{Environment.NewLine}\t\tMsgBox \"오류 내용 : \" & Err.Description, vbCritical, \"오류\"{Environment.NewLine}\tEnd If", "오류 처리문", "VBA에서 발생하는 오류를 처리 할 수 있는 핸들러를 만듭니다.",
                               ResourceImage.GetIconImage("DeclaratorIcon"), CompletionFlag.Declarator | CompletionFlag.General),
                           ...
                           // 설명 다시 작성
                           new CompletionData("If", "If", "If문을 작성합니다.",
                               ResourceImage.GetIconImage("DeclaratorIcon"), CompletionFlag.General),
                           new CompletionData("Select Case","Select Case","Select Case문을 작성합니다.",
                               ResourceImage.GetIconImage("DeclaratorIcon"), CompletionFlag.General),
                           new CompletionData("Case","Case","Select Case문의 Case를 작성합니다.",
                               ResourceImage.GetIconImage("DeclaratorIcon"), CompletionFlag.General),



                           new CompletionData("End","End", "블록을 종료합니다.",
                               ResourceImage.GetIconImage("DeclaratorIcon"), CompletionFlag.General),
