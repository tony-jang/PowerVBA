    ...
    class VBACompletionEngine
    {
        ...
        public IEnumerable<ICompletionData> GetCompletionData(int offset, bool controlSpace)
        {
            ...
            if (flags.HasFlag(VisibleItem.Class))
            {
                ...
                {
                    var itm = new CompletionData(compStr, compStr, "Class " + compStr, ResourceImage.GetResourceImage("ClassIcon"), CompletionFlag.None);
            ...
            if (flags.HasFlag(VisibleItem.Type))
            {
                var itms = (new CompletionData[] {
                           new CompletionData("String", "String", "문자들의 모음입니다.",
                               ResourceImage.GetResourceImage("DeclaratorIcon"), CompletionFlag.None),
            ...
            if (flags.HasFlag(VisibleItem.Declarer))
            {
                var itms = (new CompletionData[] {
                           new CompletionData("Dim", "Dim", "하나 이상의 변수에 사용할 저장 공간을 선언하고 할당합니다.",
                               ResourceImage.GetResourceImage("DeclaratorIcon"), CompletionFlag.Declarator | CompletionFlag.General),
                           ...
                           new CompletionData("As", "As", "선언문의 데이터 형식을 지정합니다.",
                               ResourceImage.GetResourceImage("DeclaratorIcon"), CompletionFlag.Declarator | CompletionFlag.As),

                           new CompletionData("Sub", "Sub", "호출 코드에 값을 반환하지 않는 프로시저인 Sub 프로시저를 정의하는 이름, 매개 변수 및 코드를 선언합니다.",
                               ResourceImage.GetResourceImage("DeclaratorIcon"), CompletionFlag.Declarator | CompletionFlag.General | CompletionFlag.Naming),
                           ...
                           new CompletionData("Class", "Class", "클래스 이름을 선언하고 클래스를 구성하는 변수, 속성 및 메서드의 정의를 지정합니다.",
                               ResourceImage.GetResourceImage("DeclaratorIcon"), CompletionFlag.Naming | CompletionFlag.General),

                           new CompletionData("Enum", "Enum", "열거형을 선언하고 열거형의 멤버 값을 정의합니다.",
                               ResourceImage.GetResourceImage("DeclaratorIcon"), CompletionFlag.Naming | CompletionFlag.General),

                           new CompletionData("With","With","단일 개채 또는 구조체를 참조하는 일련의 문을 실행합니다.",
                               ResourceImage.GetResourceImage("DeclaratorIcon"), CompletionFlag.General),

                           new CompletionData("End","End", "블록을 종료합니다.",ResourceImage.GetResourceImage("DeclaratorIcon"), CompletionFlag.General),
